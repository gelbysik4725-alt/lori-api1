...
$db['settings'] = array_merge([
    ...
    'global_freeze' => false,
    'version' => '9.0.0',
    'checksum' => 'lori_v9_clean',
    'download_url' => '',
    'emergency_msg' => '',
    'broadcast' => '',
    'panel_bg' => '',
    'panel_bg_color' => '#0f172a', // Dark Blue/Black
    'panel_accent' => '#6366f1',   // Indigo
    'panel_overlay' => '0.85',
    'panel_blur' => '20',
    'bot_welcome' => "👋 <b>LORI v9</b>\nВыберите действие:",
...
function ico($name, $size=18) {
    ...
    $map = [
        'copy'=>'<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="'.$c.'" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15V5a2 2 0 012-2h10"/></svg>',
        'user'=>'<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="'.$c.'" stroke-width="2"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 4-6 8-6s8 2 8 6"/></svg>',
        'warn'=>'<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="'.$c.'" stroke-width="2"><path d="M12 3L2 21h20L12 3z"/><path d="M12 10v5M12 18h.01"/></svg>',
        'reset'=>'<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="'.$c.'" stroke-width="2"><path d="M3 12a9 9 0 103-6.7"/><path d="M3 4v5h5"/></svg>',
        'link'=>'<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="'.$c.'" stroke-width="2"><path d="M10 13a5 5 0 007 0l2-2a5 5 0 00-7-7l-1 1"/><path d="M14 11a5 5 0 00-7 0l-2 2a5 5 0 007 7l1-1"/></svg>',
        'refresh'=>'<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="'.$c.'" stroke-width="2"><path d="M21 12a9 9 0 11-2.6-6.2"/><path d="M21 3v6h-6"/></svg>',
        'lock'=>'<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="'.$c.'" stroke-width="2"><rect x="5" y="11" width="14" height="10" rx="2"/><path d="M8 11V8a4 4 0 018 0v3"/></svg>',
        'eye'=>'<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="'.$c.'" stroke-width="2"><path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7S1 12 1 12z"/><circle cx="12" cy="12" r="3"/></svg>',
        'plane'=>'<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="'.$c.'" stroke-width="2"><path d="M22 2L11 13M22 2l-7 20-4-9-9-4 20-7z"/></svg>',
        ...
        'clock'=>'<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="'.$c.'" stroke-width="2"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg>',
        ...
        'spark'=>'<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="'.$c.'" stroke-width="2"><path d="M12 2v4M12 18v4M4.9 4.9l2.8 2.8M16.3 16.3l2.8 2.8M2 12h4M18 12h4M4.9 19.1l2.8-2.8M16.3 7.7l2.8-2.8"/></svg>',
        'globe'=>'<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="'.$c.'" stroke-width="2"><circle cx="12" cy="12" r="9"/><path d="M3 12h18M12 3a14 14 0 010 18M12 3a14 14 0 000 18"/></svg>',
        'db'=>'<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="'.$c.'" stroke-width="2"><ellipse cx="12" cy="5" rx="8" ry="3"/><path d="M4 5v6c0 1.7 3.6 3 8 3s8-1.3 8-3V5M4 11v6c0 1.7 3.6 3 8 3s8-1.3 8-3v-6"/></svg>',
        'search'=>'<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="'.$c.'" stroke-width="2"><circle cx="11" cy="11" r="7"/><path d="M21 21l-4.3-4.3"/></svg>',
        ...
        'upload'=>'<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="'.$c.'" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 5 17 10"/><line x1="12" y1="5" x2="12" y2="15"/></svg>',
        ...
        'ban'=>'<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="'.$c.'" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/></svg>',
...
*{margin:0;padding:0;box-sizing:border-box}body{min-height:100vh;display:flex;align-items:center;justify-content:center;background:#0f172a;font-family:Inter,system-ui;color:#fff}
.box{width:100%;max-width:380px;padding:40px;background:rgba(30,41,59,0.7);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.1);border-radius:24px;text-align:center}
.logo{font-size:28px;font-weight:800;background:linear-gradient(135deg,#6366f1,#a855f7);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:24px}
input{width:100%;padding:14px;background:rgba(0,0,0,0.3);border:1px solid rgba(255,255,255,0.1);border-radius:12px;color:#fff;margin-bottom:16px;outline:none}
input:focus{border-color:#6366f1}
button{width:100%;padding:14px;background:linear-gradient(135deg,#6366f1,#a855f7);border:none;border-radius:12px;color:#fff;font-weight:600;cursor:pointer}
.err{color:#ef4444;font-size:13px;margin-bottom:16px}</style></head><body>
<div class="box"><div class="logo">LORI v9</div>
...
<button type="submit">Enter System</button></form></div></body></html>';
        exit;
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
        ...
        if ($act==='set_panel_bg') {
            ...
            $db['settings']['panel_overlay']=max(0.1,min(0.95,(float)($_POST['panel_overlay']??0.85)));
            $db['settings']['panel_blur']=max(0,min(50,(int)($_POST['panel_blur']??20)));
...
:root{--bg:<?= htmlspecialchars($panelBgColor) ?>;--card:rgba(30,41,59,0.6);--border:rgba(255,255,255,0.08);--accent:<?= htmlspecialchars($accent) ?>;--accent-rgb:<?= implode(',',$rgb) ?>;--text:#f8fafc;--muted:#94a3b8}
...
<?php else: ?>
body::before{content:'';position:fixed;inset:0;z-index:-1;background:radial-gradient(circle at 20% 20%,rgba(var(--accent-rgb),0.15),transparent 40%),radial-gradient(circle at 80% 80%,rgba(168,85,247,0.1),transparent 40%)}
<?php endif; ?>
.header{background:rgba(15,23,42,0.8);padding:16px 24px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid var(--border);position:sticky;top:0;z-index:100;backdrop-filter:blur(20px)}
.header h1{font-size:14px;color:var(--accent);letter-spacing:4px;font-weight:700;display:flex;align-items:center;gap:10px}
.header a{color:var(--muted);text-decoration:none;margin-left:16px;font-size:12px;transition:.2s}
.header a:hover{color:var(--text)}
.layout{display:flex;max-width:1400px;margin:0 auto}
.sidebar{width:220px;padding:20px 12px;border-right:1px solid var(--border);min-height:calc(100vh - 65px);background:rgba(15,23,42,0.4);backdrop-filter:blur(10px)}
.sidebar a{display:flex;align-items:center;gap:10px;padding:10px 14px;border-radius:10px;color:var(--muted);text-decoration:none;margin-bottom:4px;font-size:13px;transition:.2s}
.sidebar a:hover{background:rgba(255,255,255,0.05);color:var(--text)}
.sidebar a.active{background:linear-gradient(90deg,rgba(var(--accent-rgb),0.2),transparent);color:var(--accent);border-left:3px solid var(--accent)}
.content{flex:1;padding:24px}
.stats{display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:16px;margin-bottom:24px}
.stat{background:var(--card);border:1px solid var(--border);border-radius:16px;padding:20px;text-align:center;backdrop-filter:blur(10px)}
.stat .num{font-size:24px;font-weight:700;color:var(--accent)}
.stat .label{font-size:11px;color:var(--muted);margin-top:6px;text-transform:uppercase;letter-spacing:1px}
.card{background:var(--card);border:1px solid var(--border);border-radius:16px;padding:24px;margin-bottom:20px;backdrop-filter:blur(10px)}
.card h2{font-size:14px;color:var(--accent);margin-bottom:18px;font-weight:600;display:flex;align-items:center;gap:10px}
.btn{display:inline-flex;align-items:center;gap:8px;padding:10px 18px;border-radius:10px;border:none;font-weight:600;cursor:pointer;font-size:12px;text-decoration:none;transition:.2s}
.btn-accent{background:linear-gradient(135deg,var(--accent),#8b5cf6);color:#fff;box-shadow:0 4px 12px rgba(var(--accent-rgb),0.3)}
.btn-accent:hover{transform:translateY(-1px);box-shadow:0 6px 16px rgba(var(--accent-rgb),0.4)}
...
input,select,textarea{padding:10px 14px;border-radius:10px;border:1px solid var(--border);background:rgba(0,0,0,0.2);color:#fff;font-size:13px;outline:none;transition:.2s}
...
.msg{background:rgba(16,185,129,0.1);border:1px solid rgba(16,185,129,0.2);padding:14px;border-radius:12px;margin-bottom:20px;color:#4ade80;font-size:13px}
...
.tool-card h3{font-size:13px;color:var(--accent);margin-bottom:14px;display:flex;align-items:center;gap:8px}
.keys-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px}
.key-mini{background:var(--card);border:1px solid var(--border);border-radius:16px;padding:18px;text-decoration:none;color:inherit;display:block;transition:.2s;position:relative;overflow:hidden}
.key-mini:hover{border-color:rgba(var(--accent-rgb),0.4);transform:translateY(-2px);box-shadow:0 10px 30px -10px rgba(0,0,0,0.5)}
.key-mini .km-top{display:flex;gap:12px;align-items:center;margin-bottom:12px}
.key-mini .km-circle{width:44px;height:44px;border-radius:12px;background:linear-gradient(135deg,rgba(var(--accent-rgb),0.2),rgba(var(--accent-rgb),0.05));display:flex;align-items:center;justify-content:center;font-weight:700;color:var(--accent);font-size:14px}
.key-mini .km-name{font-size:14px;font-weight:600;word-break:break-all}
...
.kp{max-width:500px;margin:0 auto 24px;background:var(--card);border:1px solid var(--border);border-radius:20px;overflow:hidden;backdrop-filter:blur(20px)}
.kp-head{padding:24px;display:flex;gap:16px;border-bottom:1px solid var(--border);background:linear-gradient(180deg,rgba(var(--accent-rgb),0.1),transparent)}
.kp-avatar{width:48px;height:48px;border-radius:14px;background:var(--accent);color:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:18px;flex-shrink:0}
.kp-title{font-size:18px;font-weight:700;margin-bottom:4px}
.kp-badge{font-size:10px;padding:4px 8px;border-radius:6px;background:rgba(255,255,255,0.1);color:var(--muted);font-weight:600}
.kp-section{padding:20px;border-bottom:1px solid var(--border)}
.kp-row{display:flex;justify-content:space-between;padding:8px 0;font-size:13px}
.kp-row .lbl{color:var(--muted)}
.kp-row .val{color:var(--text);font-weight:500}
.kp-btns{padding:20px;display:grid;grid-template-columns:repeat(3,1fr);gap:10px}
.kp-btn{background:rgba(255,255,255,0.03);border:1px solid var(--border);border-radius:12px;padding:14px 8px;color:var(--muted);font-size:11px;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:8px;transition:.2s}
.kp-btn:hover{background:rgba(var(--accent-rgb),0.1);border-color:rgba(var(--accent-rgb),0.3);color:var(--accent)}
.log-item{font-size:12px;padding:10px 0;border-bottom:1px solid var(--border);display:flex;gap:12px}
.log-item .t{color:var(--accent);white-space:nowrap;font-size:11px}
@media(max-width:900px){.layout{flex-direction:column}.sidebar{width:100%;display:flex;overflow-x:auto;border-right:none;border-bottom:1px solid var(--border)}.sidebar a{white-space:nowrap}}
</style>
</head>
<body>
<div class="header">
  <h1><?= ico('zap',18) ?> LORI v9</h1>
  <div>
    <span style="font-size:11px;color:<?= $githubOk?'#4ade80':'#64748b' ?>;margin-right:16px"><?= ico('github',14) ?> <?= $githubOk?'Sync ON':'Sync OFF' ?></span>
...
<div class="layout">
<div class="sidebar">
<a href="?admin&tab=dashboard" class="<?= $tab==='dashboard'?'active':'' ?>"><?= ico('home',16) ?> Dashboard</a>
<a href="?admin&tab=keys" class="<?= $tab==='keys'?'active':'' ?>"><?= ico('key',16) ?> Keys</a>
<a href="?admin&tab=generate" class="<?= $tab==='generate'?'active':'' ?>"><?= ico('plus',16) ?> Create</a>
<a href="?admin&tab=bulk" class="<?= $tab==='bulk'?'active':'' ?>"><?= ico('list',16) ?> Bulk</a>
<a href="?admin&tab=give" class="<?= $tab==='give'?'active':'' ?>"><?= ico('plane',16) ?> Give</a>
<a href="?admin&tab=online" class="<?= $tab==='online'?'active':'' ?>"><?= ico('globe',16) ?> Online</a>
<a href="?admin&tab=tools" class="<?= $tab==='tools'?'active':'' ?>"><?= ico('zap',16) ?> Tools</a>
<a href="?admin&tab=access" class="<?= $tab==='access'?'active':'' ?>"><?= ico('eye',16) ?> Logs</a>
<a href="?admin&tab=github" class="<?= $tab==='github'?'active':'' ?>"><?= ico('github',16) ?> GitHub</a>
<a href="?admin&tab=broadcast" class="<?= $tab==='broadcast'?'active':'' ?>"><?= ico('bell',16) ?> Broadcast</a>
<a href="?admin&tab=blacklist" class="<?= $tab==='blacklist'?'active':'' ?>"><?= ico('ban',16) ?> Blacklist</a>
<a href="?admin&tab=settings" class="<?= $tab==='settings'?'active':'' ?>"><?= ico('settings',16) ?> Settings</a>
<a href="?admin&tab=theme" class="<?= $tab==='theme'?'active':'' ?>"><?= ico('spark',16) ?> Theme</a>
</div>
<div class="content">
...
<?php if ($viewKey && isset($db['keys'][$viewKey])):
  ...
  $android=$kd['android_id']?:'not linked'; $resetLeft=$kd['reset_left']??0;
  $w=weekId(); $freezeUsed=$kd['freeze_week'][$w]??0;
  $freezeLimit=(int)$db['settings']['user_freeze_per_week'];
  $now=time(); $daysLeft='∞'; $expiresStr='forever'; $expClass='g'; $circleP=100;
  if(($kd['expires']??0)>0){$left=$kd['expires']-$now;if($left<=0){$daysLeft='0';$expiresStr='expired';$expClass='r';$circleP=0;}else{$daysLeft=(string)ceil($left/86400);$expiresStr=date('d-m-Y H:i',$kd['expires']);$circleP=min(100,max(5,($left/(30*86400))*100));}}
  elseif(($kd['duration']??0)>0&&($kd['first_use']??0)==0){$daysLeft=(string)ceil($kd['duration']/86400);$expiresStr='after activation';}
  if($isFrozen){$status='frozen';$sc='#60a5fa';}
  elseif(($kd['first_use']??0)==0){$status='unused';$sc='#fbbf24';}
  elseif(($kd['expires']??0)>0&&$now>$kd['expires']){$status='expired';$sc='#f87171';}
  else{$status='active';$sc='#4ade80';}
  $letter=mb_strtoupper(mb_substr($viewKey,0,1));
?>
<div style="text-align:center;margin-bottom:20px"><a href="?admin&tab=keys" class="btn btn-dark"><?= ico('list',14) ?> Back to List</a></div>
<div class="kp">
<div class="kp-head">
  ...
  <div style="flex:1;min-width:0">
    ...
    <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:6px">
      <span class="kp-badge"><?= $isNamed?'named':'standard' ?></span>
...
<div class="kp-section">
  <div class="kp-row"><span class="lbl">Expires</span><span class="val <?= $expClass ?>"><?= $expiresStr ?></span></div>
  <div class="kp-row"><span class="lbl">Owner</span><span class="val"><?= htmlspecialchars($ownerName) ?></span></div>
  ...
  <div class="kp-row"><span class="lbl">Devices</span><span class="val"><?= $used ?> / <?= $max ?></span></div>
  <div class="kp-row"><span class="lbl">Warnings</span><span class="val <?= $warns>0?'r':'' ?>"><?= $warns ?> / 3</span></div>
  <div class="kp-row"><span class="lbl">HWID Resets</span><span class="val"><?= $resetLeft ?></span></div>
  <div class="kp-row"><span class="lbl">Freezes / week</span><span class="val"><?= $freezeUsed ?> / <?= $freezeLimit ?></span></div>
</div>
<div class="kp-btns">
  <button class="kp-btn" type="button" onclick="navigator.clipboard.writeText('<?= htmlspecialchars($viewKey) ?>')"><?= ico('copy') ?><span>Copy</span></button>
  <form method="post" style="display:contents"><input type="hidden" name="action" value="set_nick"><input type="hidden" name="key" value="<?= htmlspecialchars($viewKey) ?>">
    <button class="kp-btn" type="button" onclick="let n=prompt('Nick:','<?= htmlspecialchars($kd['owner_name']??'') ?>');if(n!==null){this.form.nick.value=n;this.form.submit()}"><?= ico('user') ?><span>Nick</span><input type="hidden" name="nick"></button>
  </form>
  <form method="post" style="display:contents"><input type="hidden" name="action" value="add_warn"><input type="hidden" name="key" value="<?= htmlspecialchars($viewKey) ?>">
    <button class="kp-btn" type="submit"><?= ico('warn') ?><span>Warn <?= $warns ?>/3</span></button></form>
  <form method="post" style="display:contents"><input type="hidden" name="action" value="reset_warns"><input type="hidden" name="key" value="<?= htmlspecialchars($viewKey) ?>">
    <button class="kp-btn" type="submit"><?= ico('reset') ?><span>Reset Warns</span></button></form>
  <form method="post" style="display:contents"><input type="hidden" name="action" value="reset_hwid"><input type="hidden" name="key" value="<?= htmlspecialchars($viewKey) ?>">
    <button class="kp-btn" type="submit"><?= ico('link') ?><span>Reset HWID</span></button></form>
  <form method="post" style="display:contents"><input type="hidden" name="action" value="regen_key"><input type="hidden" name="key" value="<?= htmlspecialchars($viewKey) ?>">
    <button class="kp-btn" type="submit" onclick="return confirm('Regenerate?')"><?= ico('refresh') ?><span>Regen</span></button></form>
  <form method="post" style="display:contents"><input type="hidden" name="action" value="freeze_key"><input type="hidden" name="key" value="<?= htmlspecialchars($viewKey) ?>">
    <button class="kp-btn" type="submit"><?= ico('lock') ?><span><?= $isFrozen?'Unblock':'Block' ?></span></button></form>
  <a class="kp-btn" href="?admin&tab=access&filter=<?= urlencode($viewKey) ?>"><?= ico('eye') ?><span>Logs</span></a>
...
<div style="padding:20px">
  <form method="post" class="form-row"><input type="hidden" name="action" value="extend_key"><input type="hidden" name="key" value="<?= htmlspecialchars($viewKey) ?>"><input type="hidden" name="days" value="7">
    <button class="btn btn-dark btn-sm" type="submit"><?= ico('plus',14) ?> +7 Days</button></form>
  <form method="post" class="form-row"><input type="hidden" name="action" value="make_lifetime"><input type="hidden" name="key" value="<?= htmlspecialchars($viewKey) ?>">
    <button class="btn btn-dark btn-sm" type="submit"><?= ico('clock',14) ?> Lifetime</button></form>
  <form method="post" class="form-row"><input type="hidden" name="action" value="refill_resets"><input type="hidden" name="key" value="<?= htmlspecialchars($viewKey) ?>">
    <button class="btn btn-dark btn-sm" type="submit"><?= ico('refresh',14) ?> Refill Resets</button></form>
  <form method="post" class="form-row"><input type="hidden" name="action" value="transfer_key"><input type="hidden" name="key" value="<?= htmlspecialchars($viewKey) ?>">
    <input type="number" name="new_tg" placeholder="Transfer TG ID" style="flex:1"><button class="btn btn-dark btn-sm" type="submit">OK</button></form>
  <form method="post" onsubmit="return confirm('Delete?')" class="form-row"><input type="hidden" name="action" value="delete_key"><input type="hidden" name="key" value="<?= htmlspecialchars($viewKey) ?>">
    <button class="btn btn-red btn-sm" type="submit"><?= ico('trash',14) ?> Delete</button></form>
</div>
</div>
...
Database syncs to <code>database.json</code>.<br>
Status: <b style="color:<?= $githubOk?'#4ade80':'#f87171' ?>"><?= $githubOk?'Configured':'Not Configured' ?></b><br>
...
<div class="stat"><div class="num"><?= $namedCount ?></div><div class="label">Named</div></div>
</div>
<div class="card"><h2><?= ico('shield') ?> System Status</h2>
<div class="form-row">
...
<form method="post"><input type="hidden" name="action" value="toggle_global_freeze"><button class="btn btn-dark" type="submit"><?= ico('lock',14) ?> Freeze All</button></form>
</div></div>

<?php elseif ($tab==='tools'): ?>
<div class="tool-grid">
<div class="tool-card">
<h3><?= ico('clock') ?> Extensions</h3>
<form method="post" class="form-row"><input type="hidden" name="action" value="mass_extend_all"><input type="number" name="mass_days" value="30" style="width:70px"><button class="btn btn-accent" type="submit">+Days All</button></form>
<form method="post" class="form-row" onsubmit="return confirm('All lifetime?')"><input type="hidden" name="action" value="mass_lifetime_all"><button class="btn btn-accent" type="submit">Lifetime All</button></form>
</div>
<div class="tool-card"><h3><?= ico('zap') ?> Mass Actions</h3>
...
<form method="post" class="form-row"><input type="hidden" name="action" value="strip_all_devices"><button class="btn btn-dark btn-sm" type="submit">Reset All HWID</button></form>
<form method="post" class="form-row"><input type="hidden" name="action" value="delete_expired"><button class="btn btn-red btn-sm" type="submit">Delete Expired</button></form>
</div>
<div class="tool-card"><h3><?= ico('plane') ?> Notify</h3>
<form method="post"><input type="hidden" name="action" value="notify_owners"><textarea name="notify_text"></textarea>
<button class="btn btn-accent btn-sm" type="submit" style="margin-top:8px">Send</button></form>
</div>
<div class="tool-card"><h3><?= ico('upload') ?> Import</h3>
<form method="post"><input type="hidden" name="action" value="import_keys"><textarea name="import_text" placeholder="One key per line"></textarea>
<div class="form-row" style="margin-top:8px"><input type="number" name="import_hours" value="24" style="width:70px"><button class="btn btn-accent btn-sm" type="submit">Import</button></div></form>
</div>
<div class="tool-card"><h3><?= ico('download') ?> Export</h3>
...
<div class="card"><h2><?= ico('eye') ?> Access Logs</h2>
<?php
...
<?php elseif ($tab==='keys'): ?>
<div class="card"><h2><?= ico('key') ?> Keys (<?= $totalKeys ?>)</h2>
<input type="text" id="searchKey" placeholder="Search..." onkeyup="filterKeys()" style="max-width:300px;margin-bottom:16px;width:100%">
...
<div class="card"><h2><?= ico('plus') ?> Create Key</h2>
<form method="post"><input type="hidden" name="action" value="gen_key">
<div class="form-row"><input type="text" name="custom_name" placeholder="Custom Name (Optional)" style="width:300px"></div>
<div class="form-row">
<input type="number" name="hours" value="24" style="width:100px">
<input type="number" name="max" value="1" min="1" style="width:60px">
...
<button class="btn btn-accent" type="submit"><?= ico('check',14) ?> Create</button>
</div></form></div>

<?php elseif ($tab==='bulk'): ?>
<div class="card"><h2><?= ico('list') ?> Bulk Generate</h2>
...
<?php elseif ($tab==='give'): ?>
<div class="card"><h2><?= ico('plane') ?> Give Key</h2>
<form method="post"><input type="hidden" name="action" value="give_key">
<div class="form-row">
<input type="number" name="tg_id" placeholder="TG ID" required style="width:150px">
<input type="text" name="custom_name" placeholder="Custom Name" style="width:120px">
...
<div class="card"><h2><?= ico('globe') ?> Online (<?= $onlineCount ?>)</h2>
<?php if(empty($db['online'])):?><p style="color:var(--muted)">Empty</p><?php else:?>
<table><tr><th>Key</th><th>IP</th><th>HWID</th><th>Ping</th></tr>
...
<button class="btn btn-accent" type="submit" style="margin-top:8px">Save</button></form></div>

<?php elseif ($tab==='blacklist'): ?>
<div class="card"><h2><?= ico('ban') ?> Blacklist</h2>
...
<div class="card"><h2><?= ico('settings') ?> Settings</h2>
...
<div class="form-row"><label style="width:160px">HWID Resets</label><input type="number" name="user_hwid_resets" value="<?= (int)$db['settings']['user_hwid_resets'] ?>" style="width:70px"></div>
<div class="form-row"><label style="width:160px">Freeze/Week</label><input type="number" name="user_freeze_per_week" value="<?= (int)$db['settings']['user_freeze_per_week'] ?>" style="width:70px"></div>
<div class="form-row"><label><input type="checkbox" name="purchases_enabled" value="1" <?= !empty($db['settings']['purchases_enabled'])?'checked':'' ?>> Bot Purchases</label></div>
...
<div class="card"><h2><?= ico('spark') ?> Customization</h2>
<form method="post"><input type="hidden" name="action" value="set_panel_bg">
<div class="form-row"><label style="width:120px">Background URL</label><input type="text" name="panel_bg" value="<?= htmlspecialchars($panelBg) ?>" style="flex:1"></div>
<div class="form-row"><label style="width:120px">Base Color</label><input type="color" name="panel_bg_color" value="<?= htmlspecialchars($panelBgColor) ?>"></div>
<div class="form-row"><label style="width:120px">Accent Color</label><input type="color" name="panel_accent" value="<?= htmlspecialchars($accent) ?>"></div>
<div class="form-row"><label style="width:120px">Blur Strength</label><input type="range" name="panel_blur" min="0" max="50" value="<?= $blur ?>" style="flex:1"></div>
<div class="form-row"><label style="width:120px">Overlay Opacity</label><input type="range" name="panel_overlay" min="0.1" max="0.95" step="0.01" value="<?= htmlspecialchars($overlay) ?>" style="flex:1"></div>
<button class="btn btn-accent" type="submit">Apply Theme</button>
</form></div>

<?php elseif ($tab==='logs'): ?>
<div class="card"><h2><?= ico('list') ?> Logs</h2>
...
if (isset($update['message'])) {
    ...
    if ($isAdmin && strpos($text,'/gen ')===0) {
        ...
        $duration=$hours===0?0:$hours*3600;
        if($name){ if(isset($db['keys'][$name])){sendMessage($chatId,'Taken');exit;} $db['keys'][$name]=makeKeyData($duration,$max,$level,0,$name,true); $newKey=$name; }
    ...
    if ($text==='/start' || $text==='/menu') {
        $kb=['inline_keyboard'=>[
            [['text'=>'1h · 10★','callback_data'=>'buy_1_1']],
            [['text'=>'24h · 25★','callback_data'=>'buy_24_1']],
            [['text'=>'7d · 75★','callback_data'=>'buy_168_1']],
            [['text'=>'30d · 125★','callback_data'=>'buy_720_1']],
            [['text'=>'Lifetime · 400★','callback_data'=>'buy_0_1']],
            [['text'=>'My Keys','callback_data'=>'my_keys'],['text'=>'Limits','callback_data'=>'limits_info']]
        ]];
...
if (isset($update['callback_query'])) {
    ...
    if (strpos($data,'buy_')===0) {
        if(empty($db['settings']['purchases_enabled'])){answerCallback($cqId,'Disabled',true);exit;}
    ...
    if ($data==='limits_info') {
        answerCallback($cqId);
        sendMessage($chatId,"<b>Limits</b>\n\nHWID Reset: <b>".((int)$db['settings']['user_hwid_resets'])."</b> total\nFreeze: <b>".((int)$db['settings']['user_freeze_per_week'])."</b> / week");
        exit;
    }
    if ($data==='my_keys') {
        $f=false;
        foreach($db['keys'] as $k=>$kd){
            ...
            $st=!empty($kd['is_frozen'])?'Frozen':((($kd['expires']??0)==0)?'∞':ceil(max(0,$kd['expires']-time())/86400).'d');
            sendMessage($chatId,"<b>LORI</b>\n<code>$k</code>\n$st · $used/$max\nResets: $resetLeft · Freeze: $fu/$fl",[
                'inline_keyboard'=>[
                    [['text'=>"Reset HWID ($resetLeft)",'callback_data'=>'user_reset_'.$k]],
                    [['text'=>!empty($kd['is_frozen'])?"Unfreeze ($fu/$fl)":"Freeze ($fu/$fl)",'callback_data'=>'user_freeze_'.$k]]
                ]
            ]);
        }
        if(!$f) sendMessage($chatId,'No keys');
    ...
    if (strpos($data,'user_reset_')===0) {
        ...
        if(isset($db['keys'][$key])&&($db['keys'][$key]['owner_tg']??0)==$chatId){
            if(($db['keys'][$key]['reset_left']??0)>0){
                ...
                answerCallback($cqId,'OK, left '.$db['keys'][$key]['reset_left']);
            } else answerCallback($cqId,'Limit',true);
        }
        exit;
    }
    if (strpos($data,'user_freeze_')===0) {
        ...
        if(isset($db['keys'][$key])&&($db['keys'][$key]['owner_tg']??0)==$chatId){
            $kd=&$db['keys'][$key];
            if(!empty($kd['is_frozen'])){ $kd['is_frozen']=false; saveDb(); answerCallback($cqId,'Unfrozen'); }
            else {
                if(!canUserFreeze($kd)){ answerCallback($cqId,'Weekly limit',true); exit; }
                $kd['is_frozen']=true; registerUserFreeze($kd); saveDb(); answerCallback($cqId,'Frozen');
            }
        }
        exit;
    }
    ...
    if (strpos($data,'k_view_')===0) {
        ...
        editMessage($chatId,$msgId,"<code>$key</code>\n".(count($kd['activations']??[])).'/'.($kd['max']??1),['inline_keyboard'=>[
            [['text'=>'Reset HWID','callback_data'=>'k_rhwid_'.$key],['text'=>'Freeze','callback_data'=>'k_freeze_'.$key]],
