...
function makeKeyData($duration, $max, $level, $owner_tg=0, $owner_name='', $named=false) {
    ...
    return [
        ...
        'reset_left'=>$resets,'is_frozen'=>false,'level'=>$level,'created'=>time(),
        'warns'=>0,'note'=>'','vip'=>false,'named'=>$named,'tag'=>'',
        'soft_ban_until'=>0,'android_id'=>'','freeze_week'=>[],'freeze_used_week'=>0
    ];
}
...
function canUserFreeze($kd) {
    global $db;
    $limit = !empty($kd['vip'])
        ? (int)($db['settings']['vip_freeze_per_week'] ?? 5)
        : (int)($db['settings']['user_freeze_per_week'] ?? 2);
    $w = weekId();
    $used = 0;
    if (!empty($kd['freeze_week'][$w])) $used = (int)$kd['freeze_week'][$w];
    return $used < $limit;
}
...
if ($action === 'check') {
    ...
    $max = (int)($kd['max'] ?? 1);
    if (!empty($kd['vip'])) $max += (int)($db['settings']['vip_extra_devices'] ?? 1);
    ...
    foreach ($acts as &$a) {
        if (($a['hwid'] ?? '') === $hwid) {
            ...
            $db['stats']['activations'] = ($db['stats']['activations'] ?? 0) + 1;
            saveDb();
            echo 'SUCCESS'; exit;
        }
    }
    ...
    if (count($acts) < $max) {
        ...
        addAccessLog($key, $hwid, $ip, false);
        echo 'Device limit';
    }
    exit;
}
...
function ico($name, $size=18) {
    $s = (int)$size;
    $icons = [
        'copy' => '<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15V5a2 2 0 0 1 2-2h10"/></svg>',
        'user' => '<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 4-6 8-6s8 2 8 6"/></svg>',
        'warn' => '<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 3L2 21h20L12 3z"/><path d="M12 10v5M12 18h.01"/></svg>',
        'reset' => '<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M3 12a9 9 0 1 0 3-6.7"/><path d="M3 4v5h5"/></svg>',
        'link' => '<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M10 13a5 5 0 0 0 7 0l2-2a5 5 0 0 0-7-7l-1 1"/><path d="M14 11a5 5 0 0 0-7 0l-2 2a5 5 0 0 0 7 7l1-1"/></svg>',
        'refresh' => '<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M21 12a9 9 0 1 1-2.6-6.2"/><path d="M21 3v6h-6"/></svg>',
        'lock' => '<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="5" y="11" width="14" height="10" rx="2"/><path d="M8 11V8a4 4 0 0 1 8 0v3"/></svg>',
        'eye' => '<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7S1 12 1 12z"/><circle cx="12" cy="12" r="3"/></svg>',
        'plane' => '<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M22 2L11 13"/><path d="M22 2l-7 20-4-9-9-4 20-7z"/></svg>',
        'key' => '<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="8" cy="15" r="5"/><path d="M12 12l9-9M17 3l4 4"/></svg>',
        'zap' => '<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M13 2L3 14h8l-1 8 10-12h-8l1-8z"/></svg>',
        'clock' => '<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg>',
        'star' => '<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 2l3 7h7l-5.5 4.5L18 21l-6-4-6 4 1.5-7.5L2 9h7l3-7z"/></svg>',
        'shield' => '<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 3l8 3v6c0 5-3.5 8-8 9-4.5-1-8-4-8-9V6l8-3z"/></svg>',
        'trash' => '<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M3 6h18M8 6V4h8v2M19 6l-1 14H6L5 6"/></svg>',
        'plus' => '<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 5v14M5 12h14"/></svg>',
        'check' => '<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M5 12l5 5L20 7"/></svg>',
        'x' => '<svg width="'.$s.'" height="'.$s.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M6 6l12 12M18 6L6 18"/></svg>',
    ];
    return $icons[$name] ?? '';
}
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
        ...
        if ($k && isset($db['keys'][$k])) {
            ...
            if ($act==='toggle_vip') {
                ...
                if (!empty($db['keys'][$k]['vip'])) {
                    $db['keys'][$k]['reset_left'] = max($db['keys'][$k]['reset_left']??0, (int)($db['settings']['vip_hwid_resets']??5));
                }
                saveDb(); header('Location:?admin&view='.urlencode($k).'&msg=VIP'); exit;
            }
            if ($act==='extend_key') {
            ...
            if ($act==='transfer_key') { $t=(int)($_POST['new_tg']??0); if($t>0){$db['keys'][$k]['owner_tg']=$t;saveDb();} header('Location:?admin&view='.urlencode($k).'&msg=Transfer'); exit; }
            ...
            if ($act==='refill_resets') {
                $n = !empty($db['keys'][$k]['vip']) ? (int)($db['settings']['vip_hwid_resets']??5) : (int)($db['settings']['user_hwid_resets']??2);
        ...
        if ($act==='bulk_unfreeze') { foreach($db['keys'] as &$kd)$kd['is_frozen']=false; unset($kd); saveDb(); redirectAdmin('tools','Unfreeze all'); }
        ...
        if ($act==='save_settings') {
            ...
            $db['settings']['vip_hwid_resets']=max(0,(int)($_POST['vip_hwid_resets']??5));
            $db['settings']['vip_freeze_per_week']=max(0,(int)($_POST['vip_freeze_per_week']??5));
            $db['settings']['vip_extra_devices']=max(0,(int)($_POST['vip_extra_devices']??1));
        ...
        if ($act==='clear_extend_log') { $db['extend_log']=[]; saveDb(); redirectAdmin('tools','OK'); }
        ...
        if ($act==='mass_vip') { $n=0; foreach($db['keys'] as &$kd){ if(empty($kd['vip'])){$kd['vip']=true;$kd['reset_left']=max($kd['reset_left']??0,(int)$db['settings']['vip_hwid_resets']);$n++;}} unset($kd); saveDb(); redirectAdmin('tools',"VIP $n"); }
        if ($act==='delete_expired') { $n=0; foreach($db['keys'] as $kk=>$kd){ if(($kd['expires']??0)>0&&time()>$kd['expires']){unset($db['keys'][$kk]);$n++;}} saveDb(); redirectAdmin('tools',"Удалено $n"); }
    }
...
.card h2{font-size:12px;color:var(--accent);margin-bottom:14px;font-weight:600}
...
.tool-card h3 svg{opacity:.8}
.keys-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(270px,1fr));gap:12px}
.key-mini{background:var(--card);border:1px solid var(--border);border-radius:14px;padding:14px;text-decoration:none;color:inherit;display:block}
.key-mini:hover{border-color:rgba(var(--accent-rgb),.4)}
...
.kp-badge.vip{background:var(--accent);color:#000;border:none;font-weight:600}
...
.kp-btn{background:#111113;border:1px solid #1c1c1e;border-radius:12px;padding:12px 6px;color:#9a9a9a;font-size:10px;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:6px;transition:.15s}
.kp-btn:hover{background:rgba(var(--accent-rgb),.08);border-color:rgba(var(--accent-rgb),.35);color:var(--accent)}
.kp-btn svg{opacity:.85}
.kp-btn.danger:hover{color:#f87171;border-color:rgba(248,113,113,.4)}
.kp-extra{padding:0 12px 14px}
.kp-extra .hint{font-size:10px;color:#555;margin:8px 0 4px}
...
.vip-box{background:rgba(var(--accent-rgb),.06);border:1px solid rgba(var(--accent-rgb),.2);border-radius:12px;padding:14px;margin-bottom:14px;font-size:12px;line-height:1.6;color:#aaa}
.vip-box b{color:var(--accent)}
...
<a href="?admin&tab=access" class="<?= $tab==='access'?'active':'' ?>">Логи входов</a>
...
<a href="?admin&tab=vipinfo" class="<?= $tab==='vipinfo'?'active':'' ?>">Что даёт VIP</a>
</div>
<div class="content">
...
<?php if ($viewKey && isset($db['keys'][$viewKey])):
  ...
  $isVip=!empty($kd['vip']); $isNamed=!empty($kd['named']); $isFrozen=!empty($kd['is_frozen']);
  $android=$kd['android_id']?:'не привязан'; $resetLeft=$kd['reset_left']??0;
  $w=weekId(); $freezeUsed=$kd['freeze_week'][$w]??0;
  $freezeLimit=$isVip?(int)$db['settings']['vip_freeze_per_week']:(int)$db['settings']['user_freeze_per_week'];
...
<div class="kp-head">
  ...
  <div style="flex:1;min-width:0">
    <div class="kp-title-row">
      ...
      <?php if($isVip):?><span class="kp-badge vip">VIP</span><?php endif; ?>
    </div>
    <div class="kp-sub">
...
<div class="kp-section">
  ...
  <div class="kp-row"><span class="lbl">Владелец</span><span class="val"><?= htmlspecialchars($ownerName) ?> · <?= $isNamed?'именной':'обычный' ?></span></div>
  ...
  <div class="kp-row"><span class="lbl">Сбросы HWID</span><span class="val"><?= $resetLeft ?> осталось</span></div>
  <div class="kp-row"><span class="lbl">Заморозки (неделя)</span><span class="val"><?= $freezeUsed ?> / <?= $freezeLimit ?></span></div>
</div>
<div class="kp-btns">
  ...
  <form method="post" style="display:contents"><input type="hidden" name="action" value="set_nick"><input type="hidden" name="key" value="<?= htmlspecialchars($viewKey) ?>">
    <button class="kp-btn" type="button" onclick="let n=prompt('Ник:','<?= htmlspecialchars($kd['owner_name']??'') ?>');if(n!==null){this.form.nick.value=n;this.form.submit()}"><?= ico('user') ?><span>Ник</span><input type="hidden" name="nick" value=""></button>
  </form>
  <form method="post" style="display:contents"><input type="hidden" name="action" value="add_warn"><input type="hidden" name="key" value="<?= htmlspecialchars($viewKey) ?>">
    <button class="kp-btn" type="submit"><?= ico('warn') ?><span>Варн <?= $warns ?>/3</span></button>
  </form>
  <form method="post" style="display:contents"><input type="hidden" name="action" value="reset_warns"><input type="hidden" name="key" value="<?= htmlspecialchars($viewKey) ?>">
    <button class="kp-btn" type="submit"><?= ico('reset') ?><span>Сброс варнов</span></button>
  </form>
  <form method="post" style="display:contents"><input type="hidden" name="action" value="reset_hwid"><input type="hidden" name="key" value="<?= htmlspecialchars($viewKey) ?>">
    <button class="kp-btn" type="submit"><?= ico('link') ?><span>Сброс HWID</span></button>
  </form>
  <form method="post" style="display:contents"><input type="hidden" name="action" value="regen_key"><input type="hidden" name="key" value="<?= htmlspecialchars($viewKey) ?>">
    <button class="kp-btn" type="submit" onclick="return confirm('Перегенерировать?')"><?= ico('refresh') ?><span>Перегенер.</span></button>
  </form>
  <form method="post" style="display:contents"><input type="hidden" name="action" value="freeze_key"><input type="hidden" name="key" value="<?= htmlspecialchars($viewKey) ?>">
    <button class="kp-btn" type="submit"><?= ico('lock') ?><span><?= $isFrozen?'Разблок':'Блок' ?></span></button>
  </form>
  <a class="kp-btn" href="?admin&tab=access&filter=<?= urlencode($viewKey) ?>"><?= ico('eye') ?><span>Логи</span></a>
  <form method="post" style="display:contents"><input type="hidden" name="action" value="toggle_vip"><input type="hidden" name="key" value="<?= htmlspecialchars($viewKey) ?>">
    <button class="kp-btn" type="submit"><?= ico('star') ?><span><?= $isVip?'VIP ✓':'VIP' ?></span></button>
  </form>
</div>
<div class="kp-extra">
...
<div style="text-align:center;margin-bottom:14px"><a href="?admin&tab=keys" class="btn btn-dark">← Назад</a></div>
...
<?php elseif ($tab==='vipinfo'): ?>
<div class="vip-box">
<b>Что даёт VIP</b><br>
• Больше сбросов HWID: <b><?= (int)$db['settings']['vip_hwid_resets'] ?></b> (обычно <?= (int)$db['settings']['user_hwid_resets'] ?>)<br>
• Больше заморозок в неделю: <b><?= (int)$db['settings']['vip_freeze_per_week'] ?></b> (обычно <?= (int)$db['settings']['user_freeze_per_week'] ?>)<br>
• Доп. устройство: <b>+<?= (int)$db['settings']['vip_extra_devices'] ?></b> к лимиту<br>
• Значок VIP на карточке ключа<br>
• Приоритет при поддержке (метка в панели)
</div>
<div class="card"><h2>Лимиты владельца</h2>
<p style="color:#aaa;font-size:13px;line-height:1.7">
Обычный ключ: <b><?= (int)$db['settings']['user_hwid_resets'] ?></b> сброса HWID на весь срок · <b><?= (int)$db['settings']['user_freeze_per_week'] ?></b> заморозки в неделю<br>
VIP: <b><?= (int)$db['settings']['vip_hwid_resets'] ?></b> сброса · <b><?= (int)$db['settings']['vip_freeze_per_week'] ?></b> заморозки/нед · +<?= (int)$db['settings']['vip_extra_devices'] ?> устройство
</p></div>
...
<form method="post"><input type="hidden" name="action" value="toggle_global_freeze"><button class="btn btn-dark" type="submit">Global Freeze</button></form>
</div></div>
...
<form method="post"><input type="hidden" name="action" value="notify_owners"><textarea name="notify_text" placeholder="Текст..."></textarea>
<button class="btn btn-accent btn-sm" type="submit" style="margin-top:8px">Отправить</button></form>
</div>
<div class="tool-card"><h3><?= ico('key') ?> Импорт</h3>
...
<div class="tool-card"><h3><?= ico('zap') ?> Экспорт</h3>
<a class="btn btn-dark btn-sm" href="?admin&action=export_keys">TXT</a>
<a class="btn btn-dark btn-sm" href="?admin&action=export_json">JSON</a>
</div>
</div>

<?php elseif ($tab==='access'): ?>
<div class="card"><h2>Логи входов<?= !empty($_GET['filter'])?' · '.htmlspecialchars($_GET['filter']):'' ?></h2>
<?php
$filter=$_GET['filter']??'';
$logs=$db['access_log'];
...
<select name="level"><option value="premium" selected>Premium</option></select>
<button class="btn btn-accent" type="submit">Generate</button>
</div></form></div>

<?php elseif ($tab==='give'): ?>
<div class="card"><h2>Выдать</h2>
...
<div class="card"><h2>Настройки + лимиты</h2>
...
<div class="form-row"><label style="width:160px">Сбросы HWID (обычный)</label><input type="number" name="user_hwid_resets" value="<?= (int)$db['settings']['user_hwid_resets'] ?>" style="width:70px"></div>
<div class="form-row"><label style="width:160px">Заморозки/нед (обычный)</label><input type="number" name="user_freeze_per_week" value="<?= (int)$db['settings']['user_freeze_per_week'] ?>" style="width:70px"></div>
<div class="form-row"><label style="width:160px">Сбросы HWID (VIP)</label><input type="number" name="vip_hwid_resets" value="<?= (int)$db['settings']['vip_hwid_resets'] ?>" style="width:70px"></div>
<div class="form-row"><label style="width:160px">Заморозки/нед (VIP)</label><input type="number" name="vip_freeze_per_week" value="<?= (int)$db['settings']['vip_freeze_per_week'] ?>" style="width:70px"></div>
<div class="form-row"><label style="width:160px">VIP +устройства</label><input type="number" name="vip_extra_devices" value="<?= (int)$db['settings']['vip_extra_devices'] ?>" style="width:70px"></div>
...
<div class="form-row"><label style="width:120px">Цвет фона</label><input type="color" name="panel_bg_color" value="<?= htmlspecialchars($panelBgColor) ?>"></div>
...
if (isset($update['message']['successful_payment'])) {
    ...
    sendMessage($chatId,"<b>LORI</b>\nОплата прошла.\n\nКлюч:\n<code>$newKey</code>\n\nСбросы HWID: 2 на весь срок\nЗаморозки: 2 в неделю");
    exit;
}
...
if (isset($update['message'])) {
    ...
    if ($isAdmin && strpos($text,'/gen ')===0) {
        ...
        $duration=$hours===0?0:$hours*3600;
        if($name){ if(isset($db['keys'][$name])){sendMessage($chatId,'Имя занято');exit;} $db['keys'][$name]=makeKeyData($duration,$max,$level,0,$name,true); $newKey=$name; }
    ...
    if ($text==='/start' || $text==='/menu') {
        $kb=['inline_keyboard'=>[
            ...
            [['text'=>'Мои ключи','callback_data'=>'my_keys'],['text'=>'VIP бонусы','callback_data'=>'vip_info']],
...
if (isset($update['callback_query'])) {
    ...
    if (strpos($data,'buy_')===0) {
        ...
        $h=(int)explode('_',$data)[1]; $m=[1=>10,24=>25,72=>50,168=>75,720=>125,0=>400];
        sendInvoice($chatId,'LORI Key','Access',"sub_{$h}_1",$m[$h]??25); answerCallback($cqId); exit;
    }
    if ($data==='vip_info') {
        answerCallback($cqId);
        sendMessage($chatId,"<b>VIP даёт:</b>\n• ".((int)$db['settings']['vip_hwid_resets'])." сбросов HWID (вместо ".((int)$db['settings']['user_hwid_resets']).")\n• ".((int)$db['settings']['vip_freeze_per_week'])." заморозок в неделю\n• +".((int)$db['settings']['vip_extra_devices'])." устройство\n• Значок VIP");
        exit;
    }
    if ($data==='limits_info') {
        answerCallback($cqId);
        sendMessage($chatId,"<b>Лимиты владельца</b>\n\nСброс HWID: <b>".((int)$db['settings']['user_hwid_resets'])."</b> за весь срок ключа\nЗаморозка: <b>".((int)$db['settings']['user_freeze_per_week'])."</b> раз в неделю\n\nVIP увеличивает эти лимиты.");
        exit;
    }
    if ($data==='my_keys') {
        $f=false;
        foreach($db['keys'] as $k=>$kd){
            ...
            if(!empty($kd['vip'])) $max+=(int)$db['settings']['vip_extra_devices'];
            $resetLeft=$kd['reset_left']??0;
            $w=weekId(); $fu=$kd['freeze_week'][$w]??0;
            $fl=!empty($kd['vip'])?(int)$db['settings']['vip_freeze_per_week']:(int)$db['settings']['user_freeze_per_week'];
            if(!empty($kd['is_frozen'])) $st='Заморожен';
            elseif(($kd['expires']??0)==0) $st='Навсегда';
            elseif(time()>($kd['expires']??0)) $st='Истёк';
            else $st=ceil(($kd['expires']-time())/86400).' дн';
            $vip = !empty($kd['vip'])?' · VIP':'';
            $kb=['inline_keyboard'=>[
            ...
            sendMessage($chatId,"<b>LORI</b>\n<code>$k</code>$vip\nСтатус: $st\nУстройства: $used/$max\nСбросы: $resetLeft\nЗаморозки на неделе: $fu/$fl", $kb);
        }
        if(!$f) sendMessage($chatId,"Ключей нет");
    ...
    if (strpos($data,'user_reset_')===0) {
        ...
        if(isset($db['keys'][$key])&&($db['keys'][$key]['owner_tg']??0)==$chatId){
            if(($db['keys'][$key]['reset_left']??0)>0){
                ...
                answerCallback($cqId,'HWID сброшен. Осталось: '.$db['keys'][$key]['reset_left']);
            } else answerCallback($cqId,'Лимит сбросов исчерпан',true);
        }
        exit;
    }
    if (strpos($data,'user_freeze_')===0) {
        ...
        if(isset($db['keys'][$key])&&($db['keys'][$key]['owner_tg']??0)==$chatId){
            ...
            if(!empty($kd['is_frozen'])){
                $kd['is_frozen']=false; saveDb(); answerCallback($cqId,'Разморожен');
            } else {
                if(!canUserFreeze($kd)){ answerCallback($cqId,'Лимит заморозок на неделю',true); exit; }
                $kd['is_frozen']=true; registerUserFreeze($kd); saveDb();
    ...
    if (strpos($data,'k_view_')===0) {
        ...
        $kd=$db['keys'][$key];
        editMessage($chatId,$msgId,"<code>$key</code>\n".(count($kd['activations']??[])).'/'.($kd['max']??1)."\nWarns ".($kd['warns']??0)."/3",['inline_keyboard'=>[
            [['text'=>'Сброс HWID','callback_data'=>'k_rhwid_'.$key],['text'=>!empty($kd['is_frozen'])?'Размор.':'Блок','callback_data'=>'k_freeze_'.$key]],
